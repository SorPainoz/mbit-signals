...
- `extension/` — phase-2 Chrome extension (MV3, no build): `manifest.json` + `background.js` + `control.html` + `control.js`. Load unpacked from `chrome://extensions`. The toolbar icon opens the control UI in a normal Chrome window (`background.js` → `chrome.windows.create`, type `normal`) because Web Bluetooth's `requestDevice` chooser cannot attach to an action popup or a popup-type window — don't move the UI back into `default_popup` or a `type: "popup"` window. Connects over the same UART protocol (`namePrefix: "BBC micro:bit"`). Subject to the Linux Web Bluetooth flag below — same Chromium engine as the debug page.
- `debug/` — WebBluetooth BLE debug tester: `index.debug.html`, served by `make serve` via `debug/serve.mjs` (dependency-free Node, no npm) on `tcp://0.0.0.0:3000` (LAN-reachable — intentional so the son's machine can reach it; keep that binding). It exposes **only** `index.debug.html`, never the rest of the directory. `discovery.py` scans BlueZ D-Bus for the micro:bit; it declares its `dbus-next` dependency via PEP 723 inline metadata, so run it with `uv run discovery.py`.
...
- No tests, no lint, no typecheck, no CI. The root `Makefile` is the whole workflow; no npm anywhere.
...
- Web Bluetooth is **off by default in Chrome on Linux** (Chromium marks it `experimental`), so `navigator.bluetooth` is `undefined` even on localhost. Fix: enable `chrome://flags/#enable-experimental-web-platform-features`, or launch Chrome with `--enable-features=WebBluetooth`. Affects both the debug page and the extension control window (same engine). The user-level launcher at `~/.local/share/applications/google-chrome.desktop` carries the flag — terminal/scripted Chrome launches don't.
- The micro:bit's Nordic UART service **reverses the standard characteristic ids** (codal `MicroBitUARTService` maps TX→`6e400002` indicate, RX→`6e400003` write). Any BLE client must write commands to `6e400003` and subscribe to `6e400002`; the common `RX=6e400002`/`TX=6e400003` convention silently fails with `GATT Error: Not supported`.
